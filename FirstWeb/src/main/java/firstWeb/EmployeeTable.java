...
public class EmployeeTable implements EmployeeDAO{
	...
	public void createEmployee(Employee emp) throws SQLException {
		Connection con = getConnection();
		PreparedStatement ps = con.
				prepareStatement("INSERT INTO EMPLOYEE (EID,NAME,AGE,EXPERIENCE,SENIORITY,SALARY,DID) VALUES (?,?,?,?,?,?,?)");
		populatePreparedStatmentFromEmployee(emp, ps);
		int rowsUpdated = ps.executeUpdate();
		System.out.println("Rows update:" + rowsUpdated);
		con.close();
	...
	public int firstID() throws SQLException {
		...
		Connection con = getConnection();
		PreparedStatement ps = con.prepareStatement("SELECT MIN(EID) FROM EMPLOYEE");
		ResultSet rs = ps.executeQuery();
		if(rs.next()) {
			int eid = rs.getInt(1);
			con.close();
	...
	public int lastID() throws SQLException {
		Connection con = getConnection();
		PreparedStatement ps = con.prepareStatement("SELECT MAX(EID) FROM EMPLOYEE");
		ResultSet rs = ps.executeQuery();
		if(rs.next()) {
			int eid = rs.getInt(1);
			con.close();
	...
	@Override
	public int nextID(int eid) throws SQLException {
		Connection con = getConnection();
		
		PreparedStatement ps = con.prepareStatement("SELECT MIN(EID) FROM EMPLOYEE WHERE EID > ?");
		ps.setInt(1, eid);
		ResultSet rs = ps.executeQuery();
		if(rs.next()) {
			eid = rs.getInt(1);
		...
		con.close();
		return eid == 0 ? lastID() : eid;
	}

	@Override
	public int previousID(int eid) throws SQLException {
		Connection con = getConnection();
		PreparedStatement ps = con.prepareStatement("SELECT MAX(EID) FROM EMPLOYEE WHERE EID < ?");
		ps.setInt(1, eid);
		ResultSet rs = ps.executeQuery();
		if(rs.next()) {
			eid = rs.getInt(1);
